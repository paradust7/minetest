...
// [7]: virtualAddress (as 32-bit int)
// [8-15]: reserved

// Packet ring buffer starts after socket data and virtual IP counter
var OFFSET_VIRTUAL_IP_COUNTER = OFFSET_SOCKET_DATA + (MAX_SOCKETS * SOCKET_ENTRY_SIZE);
var PACKET_BUFFER_START = OFFSET_VIRTUAL_IP_COUNTER + 1;
    ...
    try {
        ...
        for (var i = 0; i < MAX_PACKETS; i++) {
            ...
            if (valid === 1) {
                ...
                // Match packet if:
                // 1. Port matches AND
                // 2. Destination IP matches local bound IP OR local bound IP is 0.0.0.0 (any)
                if (pktDestPort === destPort && (destAddrInt === 0 || pktDestAddr === destAddrInt)) {
...
var SocketProxy = {
    ...
    bind: function(fd, address, port) {
        ...
        try {
            ...
            // Assign virtual IP if binding to 0.0.0.0 (the server)
            if (address === '0.0.0.0') {
                var nextVirtualIpIdx = Atomics.add(getSharedInt32(), OFFSET_VIRTUAL_IP_COUNTER, 1);
                var virtualIp = '10.0.0.' + nextVirtualIpIdx; 
                console.log('[SocketProxyShared] Assigned virtual IP ' + virtualIp + ' to server socket');
                Atomics.store(getSharedInt32(), offset + 7, ipToInt(virtualIp));
                console.log('[SocketProxyShared] Socket bound to ' + address + ':' + port + ' (Virtual IP: ' + virtualIp + ')');
            } else {
                Atomics.store(getSharedInt32(), offset + 7, 0);
    ...
    sendto: function(fd, data, destAddress, destPort) {
        ...
        var virtualIpInt = Atomics.load(getSharedInt32(), offset + 7);
        var srcAddress = intToIp(srcIpInt);
        
        // If sending to external address and we have a virtual IP, use it
        if (destAddress !== '127.0.0.1' && virtualIpInt !== 0) {
            srcAddress = intToIp(virtualIpInt);
        } else if (srcAddress === '0.0.0.0') {
            // Default to 127.0.0.1 for local traffic from 0.0.0.0
            srcAddress = '127.0.0.1';
        }
        
        // Write packet to shared ring buffer
        console.log('[SocketProxyShared] sendto: ' + srcAddress + ':' + srcPort + ' -> ' + destAddress + ':' + destPort + ' (' + data.length + ' bytes)');
